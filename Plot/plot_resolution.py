# Extract the resolution response from the fits, plot and fit a GPR to the resolution
# Plot the jet pT resolution
import os 
import pickle
import argparse
...
for i_eta_bin in range(num_eta_bins):
    ...
    #for i_pt_bin in range(2, num_pt_bins):
        ...
        if result is not None:
            result_i = [int(i_pt_bin), int(i_eta_bin), result["x"], result["std_fit"], result["std_guess"], result["std_fit_err"], result["std_guess_err"]]
...
for i_pt_bin in range(num_pt_bins):
    ...
    if result is not None:
        result_i = [int(i_pt_bin), -1, result["x"], result["std_fit"], result["std_guess"], result["std_fit_err"], result["std_guess_err"]]
        df_inclusive.loc[i_pt_bin] = result_i
    else:
        result_i = [int(i_pt_bin), -1, 0, 0, 0, 0, 0]
...
for i_eta_bin in range(num_eta_bins):
    ...
    plt.savefig(f"{output_dir}/jet_pt_resolution_eta_{i_eta_bin}.png")
